# make this 1-1 with the C++ inplementation with the same interface
import torch
import cudnn
...
def create_conv_bias_wgrad_graph(X_gpu, W_grad_gpu, Y_grad_gpu, padding, stride):
    ...
    Y_GRAD = graph.tensor(
        name="Y_GRAD", dim=Y_grad_gpu.size(), stride=Y_grad_gpu.stride(), data_type=Y_grad_gpu.dtype
    )
    ...
    wgrad_output.set_output(True).set_dim(W_grad_gpu.size()).set_data_type(W_grad_gpu.dtype).set_stride(W_grad_gpu.stride())
...
class CudnnGraphManager:
    ...
    def create_or_get_conv_bias_fprop_graph(self, X_gpu, W_gpu, B_gpu, padding, stride):
        ...
        if key not in self.graphs["fprop"]:
            graph, uids = create_conv_bias_fprop_graph(X_gpu, W_gpu, B_gpu, padding, stride)
    ...
    def create_or_get_conv_bias_wgrad_graph(self, X_gpu, W_grad_gpu, Y_grad_gpu, padding, stride):
        key = get_conv_bias_wgrad_cache_key(X_gpu, W_grad_gpu, Y_grad_gpu, padding, stride)
        if key not in self.graphs["wgrad"]:
            graph, uids = create_conv_bias_wgrad_graph(X_gpu, W_grad_gpu, Y_grad_gpu, padding, stride)
            self.graphs["wgrad"][key] = graph
            self.graph_uids["wgrad"][key] = uids
        return self.graphs["wgrad"][key], self.graph_uids["wgrad"][key]
    
    def create_or_get_conv_bias_dgrad_graph(self, X_gpu, W_gpu, Y_grad_gpu, padding, stride):
        key = get_conv_bias_dgrad_cache_key(X_gpu, W_gpu, Y_grad_gpu, padding, stride)
        if key not in self.graphs["dgrad"]:
            graph, uids = create_conv_bias_dgrad_graph(X_gpu, W_gpu, Y_grad_gpu, padding, stride)
    ...
    def dump_graph(self, graph_type, key):
        print(f"execution plan count: {self.graphs[graph_type][key].get_execution_plan_count()}")
...
def get_fwd_conv_output_dim(tensorDim, pad, filterDim, stride, dilation):
    p = int((get_fwd_conv_padded_image_dim(tensorDim, pad) - get_fwd_conv_dilated_filter_dim(filterDim, dilation))) / stride + 1;
    return (p);
...
def conv_bias_fprop(x, weight, bias, padding, stride):
    ...
    key = get_conv_bias_fprop_cache_key(x, weight, bias_v, padding, stride)
    output_shape = get_output_buffer_shape(x, weight, bias, padding, stride)
    print(f"output_shape: {output_shape}")
    Y_actual = torch.empty(output_shape, device="cuda", dtype=x.dtype, memory_format=torch.channels_last)
    graph, uids = cudnn_graph_manager.create_or_get_conv_bias_fprop_graph(x, weight, bias_v, padding, stride)
    # allocate workspace
...
def conv_bias_bprop(x, weight, grad_output, padding, stride):
    ...
    wgrad_graph, wgrad_uids = cudnn_graph_manager.create_or_get_conv_bias_wgrad_graph(x, weight, grad_output, padding, stride)
    wgrad_workspace = torch.empty(wgrad_graph.get_workspace_size(), device="cuda", dtype=torch.uint8)
    wgrad_output = torch.empty_like(weight)
    execute_wgrad(wgrad_graph, wgrad_uids, x, wgrad_output, grad_output, wgrad_workspace)
